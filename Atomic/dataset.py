...
import os
dataset = load_dataset("roneneldan/TinyStories", split={"train": "train[:10%]", "validation": "validation"})
...
def group_text(examples):
    ...
    return result
final_dataset = tokenized_dataset.map(group_text, batched=True, num_proc=os.cpu_count())
